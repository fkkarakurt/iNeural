...
private:
    ...
    void load(std::vector<std::string> &file_names, Eigen::MatrixXd &inputs, Eigen::MatrixXd &outputs)
    {
        ...
        for (int f = 0; f < file_names.size(); f++)
        {
            ...
            if (!file.is_open())
                throw iNeural::iNeuralException("Could not open file '" + file_names[f] + "' in directory '" + directory + "'.");
            for (int n = 0; n < NperFile; n++, instance++)
            {
                if (file.eof())
                    throw iNeural::iNeuralException("Reached unexpected end of file " + file_names[f] + ".");

                file.read(values, D + 1);
                if (values[0] < 0 || values[0] >= F)
                    throw iNeural::iNeuralException("Unknown class detected.");
